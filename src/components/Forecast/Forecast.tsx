...
export function Forecast() {
	...
	const { allWeatherData, futureTime, isLoading, setFutureTime } =
		useWeatherContext()
	...
	return (
		<div
			className={classNames(
				styles.wrapper,
				isLoading && styles.skeletonWrapper
			)}
		>
			{isLoading ? (
				<>
								...
								.map((hour, idx) => (
									<div className={styles.hourWithLine} key={idx}>
										{idx > 0 && (
											<Line
												type='date'
												midnightSplit={createDateInfo(hour.dt).time === '0:00'}
											/>
										)}
										<div
											className={classNames(
												styles.hour,
												futureTime?.type === 'hour' &&
													futureTime?.dt === hour.dt &&
													styles.hourSelected
											)}
											onClick={() =>
												setFutureTime(
										...
										>
											...
											<div
												className={classNames(
													styles.rain,
													!hour.rain && styles.rainHidden
												)}
											>
												{`${
													hour.rain && Math.round(hour.rain?.['1h'] * 10) / 10
												} mm`}
											</div>
										</div>
									</div>
								...
								<div className={styles.dateWithLine} key={idx}>
									...
									>
										...
										<div className={styles.temp}>
											{`${formatTemperature(
												date.temp.max
											)}° / ${formatTemperature(date.temp.min)}°`}
										</div>
										<div
											className={classNames(
												styles.rain,
												!date.rain && styles.rainHidden
											)}
										>
